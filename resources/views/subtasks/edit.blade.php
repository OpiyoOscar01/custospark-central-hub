...
<div class="py-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
    ...
    <nav class="mb-4 flex items-center space-x-2 text-sm text-gray-500">
        ...
        <a href="{{ route('tasks.show', $subtask->task_id) }}" class="hover:text-gray-700">Task Details</a>
        <i class="bi bi-chevron-right text-xs"></i>
        <span class="text-gray-900">Edit Subtask</span>
    </nav>
    ...
    <div class="bg-white shadow-lg rounded-xl border border-gray-200">
        ...
        <div class="p-6">
            <form action="{{ route('subtasks.update', $subtask) }}" method="POST" class="space-y-8">
                @csrf
                @method('PUT')
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <div>
                            ...
                            <textarea id="description" 
                                      ...
                                      rows="4" 
                                      class="block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                                      placeholder="Enter subtask description">{{ old('description', $subtask->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600 flex items-center gap-1">
                                    <i class="bi bi-exclamation-circle"></i>
                                    {{ $message }}
                        ...
                        <div>
                            ...
                            <select id="assigned_to" 
                                ...
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" {{ old('assigned_to', $subtask->assigned_to) == $user->id ? 'selected' : '' }}>
                                        {{ $user->name }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="flex items-center justify-end space-x-3 pt-6 border-t border-gray-200">
                    <a href="{{ route('tasks.show', $subtask->task_id) }}" 
